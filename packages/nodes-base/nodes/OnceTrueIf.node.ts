...
export class OnceTrueIf implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'OnceTrueIF',
		name: 'if',
		icon: 'fa:map-signs',
		group: ['transform'],
		version: 1,
		...
		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
			...
			for (dataType of dataTypes) {
				...
				for (compareData of this.getNodeParameter(`conditions.${dataType}`, itemIndex, []) as INodeParameters[]) {
                    ...
                    console.log('=============================');
                    console.log(value1, compareData.operation, value2);
                    console.log(!([undefined, null, ''].includes(value1 as string)));
			...
			if (combineOperation === 'all') {
				...
				// so it passes.
                        if (!this.getWorkflow().context[this.getNode().name + '.run_once']) {
                          this.getWorkflow().context[this.getNode().name + '.run_once'] = 1
						  returnDataTrue.unshift(item);
                        } else {
						  returnDataFalse.unshift(item);
                        }
